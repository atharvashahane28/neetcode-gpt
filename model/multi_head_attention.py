...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.heads = nn.ModuleList([self.SingleHeadAttention(embedding_dim=embedding_dim, attention_dim=self.head_size) for _ in range(num_heads)])
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        for head in self.heads:
            outputs.append(head(embedded=embedded))
        # outputs = torch.tensor(outputs)
        # Concatenation turns many (batch_size, context_length, attention_dim // num_heads) tensors into one single (batch_size, context_length, attention_dim) tensor, by concatenating along dim=2.
        outputs = torch.cat(outputs, dim=2)
        res = self.output_projection(outputs)
