...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.token_embedding_lookup = nn.Embedding(vocab_size, model_dim)
        self.positional_embedding_lookup = nn.Embedding(context_length, model_dim)
        self.transformers = nn.Sequential()
        for _ in range(num_blocks):
            self.transformers.append(self.TransformerBlock(model_dim=model_dim, num_heads=num_heads))
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        token_embeddings = self.token_embedding_lookup(context)
        positions = torch.arange(start=0, end=context.shape[1], device=context.device)
        positional_embeddings = self.positional_embedding_lookup(positions)
        embedded = token_embeddings + positional_embeddings
        embedded = self.transformers(embedded)
